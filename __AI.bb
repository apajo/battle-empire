		...
		If this\ai = True
			If this\alive
				...
				While (obs1 Or obs2 Or obs3) And try <= 32
					...
					If i = 0 Then this\angle = this\angle + 3 * TankTurnInfluence# Else this\angle = this\angle - 3 * TankTurnInfluence#
					this\airotate = 0
				Wend
				If this\angle < 0 Then this\angle = this\angle + 360 ElseIf this\angle > 360 Then this\angle = this\angle - 360
...
Function AIShoot(weapon%, from$, x, y, angle=0)
	Select weapon
		Case 1
			...
			SendText(from$, ID_SHOOT, ShortToBytes(Int(x)) + ShortToBytes(Int(y)) + ShortToBytes(Int(angle)) + ShortToBytes(bul\timid))
		Case 2
			min.mine = New mine
				min\id$ = from$
